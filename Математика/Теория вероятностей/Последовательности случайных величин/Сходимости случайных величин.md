...
1) $F_\xi(c-0)=0$
2) $F_\xi(c+0)=1$
Тогда $\xi_n \xrightarrow[]{\mathbb{P}} c$

#### Доказательство.

$F_\xi$ вырождена в точке $c$ $\Leftrightarrow$ $P(\xi = c) = 1$ $\Rightarrow$ $P(|\xi_n - c| > \varepsilon) =$ $P(\xi_n \le c - \varepsilon) + P(\xi_n > c + \varepsilon) =$ $F_{\xi_n}(c - \varepsilon) + 1 - F_{\xi_n}(c + \varepsilon)$ $\xrightarrow[\varepsilon \to 0, \quad n \to \infty]{}$ $1 + F_{\xi}(c - 0) - F_\xi(c + 0)$ $= 0$ $\Longrightarrow$ $\xi_n \xrightarrow[]{\mathbb{P}} c$

# Сходимость по [[Характеристическая функция случайной величины|характеристике]] (слабая сходимость) (б/д)
